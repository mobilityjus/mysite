...
from .forms import PostForm, CommentForm, UserFrom
...
def post_edit(request, pk):
    ...
    if request.method == 'POST':
        ...
        #updating on exixting form
        form = PostForm(request.POST, instance=post)
        if form.is_valid():
            post = form.save(commit=False)
            post.author = request.user
            post.save() # you can also leave this blank and use ()
            return redirect('post_detail', pk=post.pk)
    else:
...
def signup(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            new_user = User.object.create_user(**form.cleaned_data)
